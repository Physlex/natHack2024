...
modelserver = ModelServer(checkpoint_path="DreamDiffusion/data/checkpoint.pth", pretrain_root='DreamDiffusion/data')
ddapi = FastAPI()
ddapi.mount(
...
async def gen_img(eegdat: EEGData, request: Request):
    ...
    # return file path
    print(os.getcwd())
    results = modelserver.infer(
        "./eeg-inputs/processed_eeg_data_updated.pth", num_samples=5, ddim_steps=250
    )
    ft = "%Y-%m-%dT%H-%M-%S"
