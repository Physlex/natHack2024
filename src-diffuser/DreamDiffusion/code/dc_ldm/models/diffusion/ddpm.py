...
from dc_ldm.models.autoencoder import (
    VQModelInterface,
    IdentityFirstStage,
    AutoencoderKL,
)
...
class DDPM(pl.LightningModule):
    ...
    def generate(
        ...
        all_samples = []
        if HW is None:
            shape = (self.p_channels, self.p_image_size, self.p_image_size)
        else:
            num_resolutions = len(self.ch_mult)
            shape = (
                self.p_channels,
                HW[0] // 2 ** (num_resolutions - 1),
                HW[1] // 2 ** (num_resolutions - 1),
            )
        ...
        with model.ema_scope():
            for count, item in enumerate(zip(data["eeg"], data["image"])):
                ...
                all_samples.append(
                    torch.cat(
                        [gt_image.detach().cpu(), x_samples_ddim.detach().cpu()], dim=0
                    )
        ...
        return (
            grid,
            (255.0 * torch.stack(all_samples, 0).cpu().numpy()).astype(np.uint8),
            state,
        )
    ...
    def save_images(self, all_samples, suffix=0):
        ...
        if self.output_path is not None:
            os.makedirs(
                os.path.join(
                    self.output_path, "val", f"{self.validation_count}_{suffix}"
                ),
                exist_ok=True,
            )
                ...
                for copy_idx, img in enumerate(imgs):
                    ...
                    Image.fromarray(img).save(
                        os.path.join(
                            self.output_path,
                            "val",
                            f"{self.validation_count}_{suffix}",
    ...
    def full_validation(self, batch, state=None):
        ...
        grid, all_samples, state = self.generate(
            batch, ddim_steps=self.ddim_steps, num_samples=5, limit=None, state=state
        )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.validation_count % 5 == 0 and self.trainer.current_epoch != 0:
            ...
            # pass
            grid, all_samples, state = self.generate(
                batch, ddim_steps=self.ddim_steps, num_samples=3, limit=5
            )
