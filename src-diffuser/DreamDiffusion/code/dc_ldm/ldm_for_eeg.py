...
class eLDM:
    def __init__(self, metafile, num_voxels, device=torch.device('cpu'),
        ...
        self.device = device    
        self.model = model.to(self.device)  # Move the model to the device during initialization
            ...
            for count, item in enumerate(fmri_embedding):
                ...
                if output_path is not None:
                    ...
                    for copy_idx, img_t in enumerate(samples_t):
                        ...
                        Image.fromarray(img_t).save(os.path.join(output_path, f'./test{count}-{copy_idx}.png'))
