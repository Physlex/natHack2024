...
class ModelServer:
    def __init__(
        self, checkpoint_path: str, pretrain_root="../data", device=torch.device("cpu")
    ):
        """
    ...
    def _load_static_models_and_config(self):
        ...
        self.config.pretrain_mbm_path = os.path.join(
            self.config.root_path, "checkpoint-eeg-500.pth"
        )
        self.config.pretrain_gm_path = os.path.join(
            self.config.root_path, "gm_pretrain"
        )

        # Load pretrained model (static components)
        pretrain_mbm_metafile = torch.load(
            self.config.pretrain_mbm_path, map_location="cpu"
        )
    ...
    def _prepare_dataset_for_inference(self, eeg_file):
        ...
        splits_path = os.path.join(
            self.config.root_path, "block_splits_by_image_single_fixed.pth"
        )
        ...
        _, dataset_test = create_EEG_dataset(
            ...
            image_transform=[
                self.get_img_transform_train(),
                self.get_img_transform_test(),
            ],
            subject=4,
        )
        ...
        self.eLDM_model.num_voxels = (
            self.num_voxels
        )  # Update model with the dynamic num_voxels
    ...
    def infer(
        ...
        if self.eLDM_model is None:
            raise RuntimeError(
                "Static models are not loaded. Call _load_static_models_and_config() first."
            )
    ...
    def get_img_transform_train(self):
        ...
        return transforms.Compose(
            [self.normalize, transforms.Resize((512, 512)), self.channel_last]
        )
...
if __name__ == "__main__":
    ...
    result_image = server.infer(
        eeg_file="../data/processed_eeg_data_updated.pth", num_samples=5, ddim_steps=250
    )
    result_image.show()
