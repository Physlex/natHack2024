...
def process_voxel_ts(v, p, t=8):
    ...
    v_split = np.concatenate(
        [np.mean(f, axis=0).reshape(1, -1) for f in v_split], axis=0
    )
    # pad the num_voxels
...
class EEGDataset_r(Dataset):
    ...
    def __init__(self, eeg_signals_path, image_transform=identity):
        ...
        self.imagenet = "../eegdata_out/imageNet_images"
...
class EEGDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        image_path = os.path.join(
            self.imagenet, image_name.split("_")[0], image_name + ".JPEG"
        )
        # print(image_path)
        try:
            image_raw = Image.open(image_path).convert(
                "RGB"
            )  # Convert to PIL.Image.Image
        except FileNotFoundError as e:
            print(f"Error loading image {image_path}: {e}")
            image_raw = Image.new(
                "RGB", (256, 256), (255, 255, 255)
            )  # Placeholder image
...
class Splitter:
    ...
    def __init__(self, dataset, split_path, split_num=0, split_name="train", subject=4):
        ...
        self.split_idx = [
            ...
            if i <= len(self.dataset.data)
            and self.dataset.data[i]["eeg"].size(1) <= 1000
        ]
...
def create_EEG_dataset(
    eeg_signals_path="../data/eeg_5_95_std.pth",
