...
def butter_bandpass(lowcut, highcut, fs, order=4):
    ...
    sos = butter(
        order, [lowcut, highcut], analog=False, btype="band", fs=fs, output="sos"
    )
    return sos
...
def process_eeg_data(
    ...
    lowcut = 5  # 5 Hz
    highcut = 95  # 95 Hz
    original_fs = 1000  # Original sampling rate (adjust based on your data)
    ...
    eeg_data_resampled = resample(
        eeg_data_filtered,
        int(sampling_rate * eeg_data_filtered.shape[-1] / original_fs),
        axis=-1,
    )
    ...
    eeg_data_reshaped = eeg_data_processed[: num_samples * target_length].reshape(
        num_samples, -1, target_length
    )
    ...
    eeg_tensors = [
        pad_to_128_channels(torch.tensor(sample, dtype=torch.float32))
        for sample in eeg_data_reshaped
    ]
    ...
    processed_data = {"dataset": data_dicts, "labels": labels, "images": images}
